...
def main():
    ...
    for episode in range(1, MAX_EPOCHS+1):
        ...
        episode_loss = update_policy(stepwise_returns, log_prob_actions, optimizer)
        losses.append(episode_loss)                             
        episode_returns.append(episode_return)
        mean_episode_return = np.mean(episode_returns[-N_TRIALS:])
        if episode % PRINT_INTERVAL == 0:
            print(f'| Episode: {episode:3} | Mean Rewards: {mean_episode_return:5.1f} | Mean Loss: {sum(losses)/len(losses):.2f}')
            losses = []
